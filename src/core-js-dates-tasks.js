...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.round(delta / 86400000) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  if (currentDate.getTime() > endDate.getTime()) return false;
  return true;
}
...
function getWeekNumberByDate(date) {
  ...
  if (firstDay.getDay() === 0) shift = 7;
  const delta = Math.ceil((date - firstDay) / 86400000) + shift;
...
function getNextFridayThe13th(date) {
  const next13 = new Date(date);
  next13.setDate(13);
  while (next13.getDate()) {
    if (next13.getDay() === 5) break;
    next13.setMonth(next13.getMonth() + 1);
  }
  return next13;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const toDate = (str) => {
    const [day, month, year] = str.split('-').map((i) => parseInt(i, 10));
    return new Date(year, month - 1, day);
  };
  const toDateString = (date) => {
    return `${date.getDate().toString().padStart(2, '0')}-${(date.getMonth() + 1).toString().padStart(2, '0')}-${date.getFullYear()}`;
  };
  const day = toDate(period.start);
  const end = toDate(period.end);
  let schedule = [];
  while (day.getTime() <= end.getTime()) {
    const nextWorkDays = Array.from({ length: countWorkDays }, (_, i) => {
      const date = new Date(i * 86400000 + day.getTime());
      if (date.getTime() <= end.getTime()) {
        return toDateString(date);
      }
      return '';
    });
    schedule = [...schedule, ...nextWorkDays];
    day.setDate(day.getDate() + countWorkDays + countOffDays);
  }
  return schedule.filter((date) => date);
}
